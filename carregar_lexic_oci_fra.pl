...
# usage
# carregar_lexic.pl n|adj|adv|vblex|pr|cnjadv|top|al|antm|antf 0|1
...
# 0. ocurrències
# 1. paraula occitana
# 2. categoria gramatical occitana
# 3. paraula francesa
...
my $MOT = 'cauma';	# paraula a debugar
my $MOT = '';

my $MORF_TRACT = $ARGV[0];
unless ($MORF_TRACT) {
	print "Error: $0 n|adj|adv|vblex|pr|cnjadv|top|al|antm|antf\n";
	exit 1;
}
...
# llegeixo el fitxer fra: n, adj, adv, pr, cnjadv, top, al, antm, antf
sub llegir_dix {
...
next if $linia !~ /$MORF_TRACT/o && $MORF_TRACT ne 'top' && $MORF_TRACT ne 'al' && $MORF_TRACT ne 'antm' && $MORF_TRACT ne 'antf';
next if $linia !~ /np/o && ($MORF_TRACT eq 'top' || $MORF_TRACT eq 'al' || $MORF_TRACT eq 'antm'  || $MORF_TRACT eq 'antf');
		...
		if ($morf eq 'np') {
			if ($nfitx eq 'fra') {
				...
				|| $par eq 'États-Unis__np') {
					$morf = 'top'
				} elsif ($par eq 'Carrefour__np' 
				|| $par eq 'Linux__np' 
				|| $par eq 'Wikipedia__np'
				|| $par eq 'Jeux_olympiques__np'
				|| $par eq 'Nations_unies__np') {
					$morf = 'al'
				...
				|| $par eq 'Estats_Units__np') {
					$morf = 'top'
				} elsif ($par eq 'ABC__np' 
				|| $par eq 'Linux__np' 
				|| $par eq 'Wikipedia__np'
				|| $par eq 'Jeux_olympiques__np'
				|| $par eq 'Nacions_Unidas__np') {
					$morf = 'al'
...
print "2.5. fitxer $nfitx, $linia, par=$par, morf=$morf\n" if $MOT && $linia =~ /"$MOT"/o;
		if ($linia !~ m|r="LR"|o && $linia =~ m|alt="([^"]*)"|o) {
			my $varietat = $1;
			$varietat_oci{$morf}{$lemma}{$varietat} = 1;
print "fitxer $nfitx, $linia, varietat_oci{$morf}{$lemma}{$varietat} = 1\n" if $MOT && $linia =~ /$MOT/o;
		} else {
print "fitxer $nfitx, $linia, varietat_oci{$morf}{$lemma}{xxx} = 0\n" if $MOT && $linia =~ /$MOT/o;
print $linia, "\n" if $MOT && $linia =~ /$MOT/o;
		}

		if ($morf ne 'n' && $morf ne 'adj' && $morf ne 'adv' && $morf ne 'vblex' && $morf ne 'pr' && $morf ne 
'cnjadv' && $morf ne 'top' && $morf ne 'al' && $morf ne 'antm' && $morf ne 'antf') {
...
# llegeixo el fitxer bilingüe: n, adj, adv, cnjadv, pr, top, al, antm, antf
sub llegir_bidix {
...
my $debug = 0;
		if ($linia =~ m|<e> *<p><l>([^<]*)</l> *<r>([^<]*)</r></p><par n="l-u-adj_l|o
...
$debug = 7;
			...
			$dir = 'bi';
#print "$.: $linia\n";
#print "lemma_oci - $lemma_oci\n";
#print "lemma_fra - $lemma_fra\n";
#print "morf - $morf\n";
#print "dir - $dir\n";
$debug = 1;
...
$debug = 4;
			...
			|| $linia =~ m|<e r="LR" alt="[^"]*".*> *<p><l>([^<]*)<s n="([^"]*)".*<r>([^<]*)<s|o
			|| $linia =~ m|<e r="LR" a="[^"]*" alt="[^"]*".*> *<p><l>([^<]*)<s n="([^"]*)".*<r>([^<]*)<s|o) {
...
$debug = 5;
			...
			|| $linia =~ m|<e r="RL" alt="[^"]*".*> *<p><l>([^<]*)<s n="([^"]*)".*<r>([^<]*)<s|o
			|| $linia =~ m|<e r="RL" a="[^"]*" alt="[^"]*".*> *<p><l>([^<]*)<s n="([^"]*)".*<r>([^<]*)<s|o) {
			$lemma_oci = $1;
			$morf = $2;
			$lemma_fra = $2;
			$dir = 'rl';
...
$debug = 9;
			...
			print STDERR "Error lectura (normal) bidix en l. $.: $linia\n";
		} else {
			next;
		}
		...
		if ($morf eq 'antm' || $morf eq 'antf') {
			if ($linia =~ m|<e> *<p><l>([^<]*)<s n="np"/><s n="ant"/><s n="([^"]*)".*<r>([^<]*)<s|o
				...
				|| $linia =~ m|<e a="[^"]*" alt="[^"]*"> *<p><l>([^<]*)<s n="np"/><s n="ant"/><s n="([^"]*)".*<r>([^<]*)<s|o) {
				$lemma_oci = $1;
				$morf = 'ant' . $2;
				$lemma_fra = $3;
				$dir = 'bi';
				...
				|| $linia =~ m|<e r="LR" alt="[^"]*".*> *<p><l>([^<]*)<s n="np"/><s n="ant"/><s n="([^"]*)".*<r>([^<]*)<s|o
				|| $linia =~ m|<e r="LR" a="[^"]*" alt="[^"]*"> *<p><l>([^<]*)<s n="np"/><s n="ant"/><s n="([^"]*)".*<r>([^<]*)<s|o) {
				$lemma_oci = $1;
				$morf = 'ant' . $2;
				$lemma_fra = $3;
				$dir = 'lr';
				...
				|| $linia =~ m|<e r="RL" alt="[^"]*".*> *<p><l>([^<]*)<s n="np"/><s n="ant"/><s n="([^"]*)".*<r>([^<]*)<s|o
				|| $linia =~ m|<e r="RL" a="[^"]*" alt="[^"]*"> *<p><l>([^<]*)<s n="np"/><s n="ant"/><s n="([^"]*)".*<r>([^<]*)<s|o) {
				$lemma_oci = $1;
				$morf = 'ant' . $2;
				$lemma_fra = $2;
				$dir = 'rl';
			...
			if ($linia =~ m|<e> *<p><l>([^<]*)<s n="np"/><s n="([^"]*)".*<r>([^<]*)<s|o
				...
				|| $linia =~ m|<e a="[^"]*" alt="[^"]*"> *<p><l>([^<]*)<s n="np"/><s n="([^"]*)".*<r>([^<]*)<s|o) {
				$lemma_oci = $1;
				$morf = $2;
				$lemma_fra = $3;
				$dir = 'bi';
				...
				|| $linia =~ m|<e r="LR" alt="[^"]*".*> *<p><l>([^<]*)<s n="np"/><s n="([^"]*)".*<r>([^<]*)<s|o
				|| $linia =~ m|<e r="LR" a="[^"]*" alt="[^"]*"> *<p><l>([^<]*)<s n="np"/><s n="([^"]*)".*<r>([^<]*)<s|o) {
				$lemma_oci = $1;
				$morf = $2;
				$lemma_fra = $3;
				$dir = 'lr';
				...
				|| $linia =~ m|<e r="RL" alt="[^"]*".*> *<p><l>([^<]*)<s n="np"/><s n="([^"]*)".*<r>([^<]*)<s|o
				|| $linia =~ m|<e r="RL" a="[^"]*" alt="[^"]*"> *<p><l>([^<]*)<s n="np"/><s n="([^"]*)".*<r>([^<]*)<s|o) {
				$lemma_oci = $1;
				$morf = $2;
				$lemma_fra = $2;
				$dir = 'rl';
		...
		if ($morf ne 'n' && $morf ne 'adj' && $morf ne 'adv' && $morf ne 'vblex' && $morf ne 'pr' && $morf ne 'cnjadv' && $morf ne 'top' && $morf ne 'al' && $morf ne 'antm' && $morf ne 'antf') {
#			print STDERR "línia $.: $linia - morf $morf\n";
			next;
		}
...
print "5. bidix r_struct_rl->{$MORF_TRACT}{$MOT} = $r_struct_rl->{$MORF_TRACT}{$MOT}\n";
print "5. bidix r_struct_lr->{$MORF_TRACT}{$MOT} = $r_struct_lr->{$MORF_TRACT}{$MOT}\n";
}
...
#print "dix_fra{$morf_fra}{$lemma_fra} = $dix_fra{$morf_fra}{$lemma_fra}\n";
	...
	if (($par_fra eq 'abeille__n' || $par_fra eq 'eau__n')
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"f\"/></l><r>%s<s n=\"n\"/><s n=\"f\"/></r></p></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"m\"/></l><r>%s<s n=\"n\"/><s n=\"f\"/></r></p></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"m\"/></l><r>%s<s n=\"n\"/><s n=\"f\"/></r></p><par n=\"sp_ND\"/></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"f\"/></l><r>%s<s n=\"n\"/><s n=\"f\"/></r></p><par n=\"pl_ND\"/></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"f\"/></l><r>%s<s n=\"n\"/><s n=\"f\"/></r></p><par n=\"sp_ND\"/></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"m\"/><s n=\"sg\"/></l><r>%s<s n=\"n\"/><s n=\"f\"/><s n=\"sg\"/></r></p></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
	} elsif ($par_fra eq 'livre__n'
			&& ($par_oci eq 'personau__n' || $par_oci eq 'persona/l__n')) {
		printf $fbi "<e$lr_rl$alt$a><p><l>%s<s n=\"n\"/><s n=\"m\"/><s n=\"sg\"/></l><r>%s<s n=\"n\"/><s n=\"m\"/><s n=\"sg\"/></r></p></e>\n", $stem_oci, $stem_fra;
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"m\"/><s n=\"sg\"/></l><r>%s<s n=\"n\"/><s n=\"m\"/><s n=\"sg\"/></r></p></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
	} elsif ($par_fra eq 'abeille__n' && $par_oci eq 'deluns__n') {
		printf $fbi "<e$lr_rl$alt$a><p><l>%s<s n=\"n\"/><s n=\"m\"/></l><r>%s<s n=\"n\"/><s n=\"f\"/></r></p><par n=\"sp_ND\"/></e>\n", $stem_oci, $stem_fra;
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"m\"/></l><r>%s<s n=\"n\"/><s n=\"f\"/></r></p><par n=\"sp_ND\"/></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
...
#			|| $par_oci eq 'espanhò/l__n'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/></l><r>%s<s n=\"n\"/></r></p><par n=\"anglés_anglais\"/></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
	} elsif (($par_fra eq 'admis__n' || $par_fra eq 'épou/x__n')
			&& ($par_oci eq 'agent__n'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/></l><r>%s<s n=\"n\"/></r></p><par n=\"omanita_omanais\"/></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
...
#			|| $par_oci eq 'espanhò/l__n'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/></l><r>%s<s n=\"n\"/></r></p></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
	} elsif (($par_fra eq 'affecté__n'
			|| $par_fra eq 'administrat/eur__n'
			|| $par_fra eq 'aïeu/l__n'
			|| $par_fra eq 'ancien__n'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s</l><r>%s</r></p><par n=\"l-u-n_l\"/></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
	} elsif (($par_fra eq 'artiste__n' || $par_fra eq 'hébreu__n')
		&& $par_oci eq 'espanhò/l__n') {
		$stem_oci =~ s/l$//o;
		printf $fbi "<e$lr_rl$alt$a><p><l>%s</l><r>%s</r></p><par n=\"l-u-n_l2\"/></e>\n", $stem_oci, $stem_fra;
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s</l><r>%s</r></p><par n=\"l-u-n_l2\"/></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
	} elsif (($par_fra eq 'affecté__n'
			|| $par_fra eq 'administrat/eur__n'
			|| $par_fra eq 'aïeu/l__n'
			|| $par_fra eq 'ancien__n'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/></l><r>%s<s n=\"n\"/></r></p><par n=\"mf_GD\"/></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
	} elsif (($par_fra eq 'artiste__n' || $par_fra eq 'hébreu__n')
		&& ($par_oci eq 'agent__n'
			|| $par_oci eq 'collèg/a__n'
			|| $par_oci eq 'guardi/a__n'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"mf\"/></l><r>%s<s n=\"n\"/><s n=\"mf\"/></r></p></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"mf\"/></l><r>%s<s n=\"n\"/><s n=\"mf\"/></r></p><par n=\"sp_ND\"/></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"mf\"/><s n=\"sp\"/></l><r>%s<s n=\"n\"/><s n=\"mf\"/><s n=\"sp\"/></r></p></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"' && $alt && $alt ne ' alt="oci"';
	} elsif (($par_fra eq 'artiste__n' || $par_fra eq 'hébreu__n')
		&& ($par_oci eq 'conselhèr__n'
			|| $par_oci eq 'afilia/t__n'
			|| $par_oci eq 'alcalde__n'
...
#			|| $par_oci eq 'espanhò/l__n'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/></l><r>%s<s n=\"n\"/></r></p><par n=\"GD_mf\"/></e>\n", $stem_oci, $stem_fra if $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"'&& $alt && $alt ne ' alt="oci"';
	} elsif (($par_fra eq 'livre__n'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"m\"/></l><r>%s<s n=\"n\"/><s n=\"m\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
	} elsif (($par_fra eq 'livre__n'
		|| $par_fra eq 'brand/y__n'
		|| $par_fra eq 'cie/l__n'
		|| $par_fra eq 'latifundi/um__n'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"f\"/></l><r>%s<s n=\"n\"/><s n=\"m\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
	} elsif (($par_fra eq 'livre__n'
		|| $par_fra eq 'brand/y__n'
		|| $par_fra eq 'cie/l__n'
		|| $par_fra eq 'latifundi/um__n'
...
#		|| $par_fra eq 'causeu/r__n' 		# molts casos d'ús d'administrat/eur__n per a aparells
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"m\"/></l><r>%s<s n=\"n\"/><s n=\"m\"/></r></p><par n=\"sp_ND\"/></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"f\"/></l><r>%s<s n=\"n\"/><s n=\"m\"/></r></p><par n=\"sp_ND\"/></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"f\"/><s n=\"sg\"/></l><r>%s<s n=\"n\"/><s n=\"m\"/><s n=\"sg\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
	} elsif ($par_fra eq 'soif__n'
			&& $par_oci eq 'set__n') {
		printf $fbi "<e$lr_rl$alt$a><p><l>%s<s n=\"n\"/><s n=\"f\"/><s n=\"sg\"/></l><r>%s<s n=\"n\"/><s n=\"f\"/><s n=\"sg\"/></r></p></e>\n", $stem_oci, $stem_fra;
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"f\"/><s n=\"sg\"/></l><r>%s<s n=\"n\"/><s n=\"f\"/><s n=\"sg\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"m\"/></l><r>%s<s n=\"n\"/><s n=\"m\"/></r></p><par n=\"ND_sp\"/></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
	} elsif ($par_fra eq 'mois__n'
			&& ($par_oci eq 'jornad/a__n'
			|| $par_oci eq 'aig/ua__n'
			|| $par_oci eq 'aubèr/ja__n'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"f\"/></l><r>%s<s n=\"n\"/><s n=\"m\"/></r></p><par n=\"ND_sp\"/></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"f\"/></l><r>%s<s n=\"n\"/><s n=\"f\"/></r></p><par n=\"ND_sp\"/></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
	} elsif ($par_fra eq 'fois__n'
			&& ($par_oci eq 'conselh__n'
			|| $par_oci eq 'autob/ús__n'
			|| $par_oci eq 'castè/l__n'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"m\"/></l><r>%s<s n=\"n\"/><s n=\"f\"/></r></p><par n=\"ND_sp\"/></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"m\"/><s n=\"pl\"/></l><r>%s<s n=\"n\"/><s n=\"m\"/><s n=\"pl\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"f\"/><s n=\"pl\"/></l><r>%s<s n=\"n\"/><s n=\"m\"/><s n=\"pl\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"m\"/><s n=\"pl\"/></l><r>%s<s n=\"n\"/><s n=\"f\"/><s n=\"pl\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"f\"/><s n=\"pl\"/></l><r>%s<s n=\"n\"/><s n=\"f\"/><s n=\"pl\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"acr\"/><s n=\"m\"/></l><r>%s<s n=\"n\"/><s n=\"acr\"/><s n=\"m\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"acr\"/><s n=\"m\"/><s n=\"sp\"/></l><r>%s<s n=\"n\"/><s n=\"acr\"/><s n=\"m\"/><s n=\"sg\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"acr\"/><s n=\"m\"/><s n=\"sg\"/></l><r>%s<s n=\"n\"/><s n=\"acr\"/><s n=\"m\"/><s n=\"sp\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"acr\"/><s n=\"f\"/></l><r>%s<s n=\"n\"/><s n=\"acr\"/><s n=\"f\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"n\"/><s n=\"acr\"/><s n=\"mf\"/></l><r>%s<s n=\"n\"/><s n=\"acr\"/><s n=\"mf\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
	} else {
...
#print "dix_fra{$morf_fra}{$lemma_fra} = $dix_fra{$morf_fra}{$lemma_fra}\n";
	...
	if (($par_fra eq 'abaissant__adj'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"adj\"/></l><r>%s<s n=\"adj\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"adj\"/></l><r>%s<s n=\"adj\"/></r></p><par n=\"mf_GD\"/></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
	} elsif (($par_fra eq 'abaissant__adj'
		|| $par_fra eq 'affirmati/f__adj'
		|| $par_fra eq 'amica/l__adj'
		|| $par_fra eq 'ancien__adj'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s</l><r>%s</r></p><par n=\"l-u-adj_l\"/></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"adj\"/></l><r>%s<s n=\"adj\"/></r></p><par n=\"GD_mf\"/></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
	} elsif (($par_fra eq 'académique__adj'
		|| $par_fra eq 'hébreu__adj')
		&& ($par_oci eq 'agricòl/a__adj'
		|| $par_oci eq 'bèlg/a__adj'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s</l><r>%s</r></p><par n=\"l-u-adj_l2\"/></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"adj\"/></l><r>%s<s n=\"adj\"/></r></p><par n=\"omanita_omanais\"/></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
	} elsif (($par_fra eq 'affectueu/x__adj' || $par_fra eq 'dou/x__adj' || $par_fra eq 'vie/ux__n' || $par_fra eq 'fra/is__adj' || $par_fra eq 'anglais__adj' || $par_fra eq 'bas__adj')
		&& ($par_oci eq 'apropria/t__adj'
		|| $par_oci eq 'trabalhador__adj'
		|| $par_oci eq 'ampl/e__adj'
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"adj\"/></l><r>%s<s n=\"adj\"/></r></p><par n=\"anglés_anglais\"/></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
		...
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"adj\"/><s n=\"mf\"/><s n=\"sp\"/></l><r>%s<s n=\"adj\"/><s n=\"mf\"/><s n=\"sg\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
	} elsif ($par_fra eq 'arrière__adj' && $par_oci eq 'post__adj') {
		printf $fbi "<e$lr_rl$alt$a><p><l>%s<s n=\"adj\"/><s n=\"mf\"/><s n=\"sp\"/></l><r>%s<s n=\"adj\"/><s n=\"mf\"/><s n=\"sg\"/></r></p></e>\n", $stem_oci, $stem_fra;
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"adj\"/><s n=\"mf\"/><s n=\"sp\"/></l><r>%s<s n=\"adj\"/><s n=\"mf\"/><s n=\"sg\"/></r></p></e>\n", $stem_oci, $stem_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
	} else {
...
#print "escriure_bidix_ant ($lemma_oci, $stem_oci, $morf_oci, $lemma_fra, $stem_fra, $morf_fra, $lr_rl, $autor)\n" if $lemma_oci eq $MOT || $lemma_fra =~ /musique/o;
#print "dix_fra{$morf_fra}{$lemma_fra} = $dix_fra{$morf_fra}{$lemma_fra}\n";
	my $par_fra = $dix_fra{$morf_fra}{$lemma_fra};
	if ($lemma_fra =~ /#/o) {
		my $x = $lemma_fra;
...
#print "escriure_bidix_top ($lemma_oci, $stem_oci, $morf_oci, $lemma_fra, $stem_fra, $morf_fra, $lr_rl, $autor)\n" if $lemma_oci eq $MOT || $lemma_fra =~ /musique/o;
#print "dix_fra{$morf_fra}{$lemma_fra} = $dix_fra{$morf_fra}{$lemma_fra}\n";
	my $par_fra = $dix_fra{$morf_fra}{$lemma_fra};
	if ($lemma_fra =~ /#/o) {
		my $x = $lemma_fra;
...
print "escriure_bidix_al ($lemma_oci, $stem_oci, $morf_oci, $lemma_fra, $stem_fra, $morf_fra, $lr_rl, $autor, $var_oci, $var_gascon, $var_aran)\n" if $lemma_oci eq $MOT || $lemma_fra eq $MOT;
#print "escriure_bidix_al ($lemma_oci, $stem_oci, $morf_oci, $lemma_fra, $stem_fra, $morf_fra, $lr_rl, $autor)\n" if $lemma_oci eq $MOT || $lemma_fra =~ /musique/o;
#print "dix_fra{$morf_fra}{$lemma_fra} = $dix_fra{$morf_fra}{$lemma_fra}\n";
	my $par_fra = $dix_fra{$morf_fra}{$lemma_fra};
	if ($lemma_fra =~ /#/o) {
		my $x = $lemma_fra;
	...
	if ($par_fra eq 'Carrefour__np' && $par_oci eq 'ABC__np') {
		printf $fbi "<e$lr_rl$alt$a><p><l>%s<s n=\"np\"/><s n=\"al\"/></l><r>%s<s n=\"np\"/><s n=\"al\"/></r></p></e>\n", $stem_oci, $stem_fra;
	} elsif ($par_fra eq 'Wikipedia__np' && $par_oci eq 'Wikipedia__np') {
		printf $fbi "<e$lr_rl$alt$a><p><l>%s<s n=\"np\"/><s n=\"al\"/><s n=\"f\"/><s n=\"sg\"/></l><r>%s<s n=\"np\"/><s n=\"al\"/><s n=\"f\"/><s n=\"sg\"/></r></p></e>\n", $stem_oci, $stem_fra;
	} elsif ($par_fra eq 'Linux__np' && $par_oci eq 'Linux__np') {
		printf $fbi "<e$lr_rl$alt$a><p><l>%s<s n=\"np\"/><s n=\"al\"/><s n=\"m\"/><s n=\"sg\"/></l><r>%s<s n=\"np\"/><s n=\"al\"/><s n=\"m\"/><s n=\"sg\"/></r></p></e>\n", $stem_oci, $stem_fra;
	} elsif ($par_fra eq 'Jeux_olympiques__np' && $par_oci eq 'Jeux_olympiques__np') {
		printf $fbi "<e$lr_rl$alt$a><p><l>%s<s n=\"np\"/><s n=\"al\"/><s n=\"m\"/><s n=\"pl\"/></l><r>%s<s n=\"np\"/><s n=\"al\"/><s n=\"m\"/><s n=\"pl\"/></r></p></e>\n", $stem_oci, $stem_fra;
	} elsif ($par_fra eq 'Nations_unies__np' && $par_oci eq 'Nacions_Unidas__np') {
		printf $fbi "<e$lr_rl$alt$a><p><l>%s<s n=\"np\"/><s n=\"al\"/><s n=\"f\"/><s n=\"pl\"/></l><r>%s<s n=\"np\"/><s n=\"al\"/><s n=\"f\"/><s n=\"pl\"/></r></p></e>\n", $stem_oci, $stem_fra;
	} else {
		print STDERR "No hi ha regla per a escriure_bidix_al: par_fra = $par_fra ($lemma_fra) par_oci = $par_oci ($lemma_oci)\n";
	}
}

sub escriure_bidix {
	my ($lemma_oci, $stem_oci, $morf_oci, $varietat_oci, $lemma_fra, $stem_fra, $morf_fra, $lr_rl, $autor) = @_;
	$lr_rl = " $lr_rl" if $lr_rl;
	my $a = " a=\"$autor\"" if $autor;
	my $var_oci = 1 if $varietat_oci{$morf_oci}{$lemma_oci}{oci};
	my $var_gascon = 1 if $varietat_oci{$morf_oci}{$lemma_oci}{gascon} || $varietat_oci eq 'oci@gascon';
	my $var_aran = 1 if $varietat_oci{$morf_oci}{$lemma_oci}{aran} || $varietat_oci eq 'oci@aran';
	my $alt = '';
	if ($var_oci) {
		$alt = " alt=\"oci\"";
	} elsif ($var_gascon) {
...
print "varietat_oci{$morf_oci}{$lemma_oci}{gascon} = $varietat_oci{$morf_oci}{$lemma_oci}{gascon}, var_gascon=$var_gascon\n" if $lemma_oci eq $MOT || $lemma_fra eq $MOT;
	if ($morf_oci eq 'vblex' && $morf_fra eq 'vblex') {
		printf $fbi "<e$lr_rl$alt$a><p><l>%s<s n=\"%s\"/></l><r>%s<s n=\"%s\"/></r></p></e>\n", $stem_oci, $morf_oci, $stem_fra, $morf_fra;
		printf $fbi "<e r=\"LR\" alt=\"oci\"$a><p><l>%s<s n=\"%s\"/></l><r>%s<s n=\"%s\"/></r></p></e>\n", $stem_oci, $morf_oci, $stem_fra, $morf_fra if $alt && $alt ne ' alt="oci"' && $lr_rl ne ' i="yes"' && $lr_rl ne ' r="RL"';
	...
	} elsif ($morf_oci eq 'al' && $morf_fra eq 'al') {
		escriure_bidix_al ($lemma_oci, $stem_oci, $morf_oci, $lemma_fra, $stem_fra, $morf_fra, $lr_rl, $autor, $var_oci, $var_gascon, $var_aran);
...
sub tractar_parella {
	my ($lemma_oci, $stem_oci, $morf_oci, $varietat_oci, $lemma_fra, $stem_fra, $morf_fra, $autor, $primer, $n_linia) = @_;
...
#print "77b. lemma_fra = $lemma_fra, lemma_fra2 = $lemma_fra2\n" if $MOT && $lemma_fra eq $MOT;
	...
	if ($GEN_FRA && !$dix_fra{$morf_fra}{$lemma_fra}) {
		...
		if ($morf_fra eq 'n' && 0) {
			...
			if ($dix_oci{$morf_oci}{$lemma_oci} eq 'Bulgaria__np') {
				printf $ffra "<e lm=\"%s\"$a><i>%s</i><par n=\"%s\"/></e>\n", $lemma_fra, $stem_fra, 'Bulgarie__np';
				$dix_fra{$morf_fra}{$lemma_fra} = 'Bulgarie__np';
			} elsif ($dix_oci{$morf_oci}{$lemma_oci} eq 'Iran__np') {
				printf $ffra "<e lm=\"%s\"$a><i>%s</i><par n=\"%s\"/></e>\n", $lemma_fra, $stem_fra, 'Iran__np';
				$dix_fra{$morf_fra}{$lemma_fra} = 'Iran__np';
			} elsif ($dix_oci{$morf_oci}{$lemma_oci} eq 'Bahamas__np') {
				printf $ffra "<e lm=\"%s\"$a><i>%s</i><par n=\"%s\"/></e>\n", $lemma_fra, $stem_fra, 'Bahamas__np';
				$dix_fra{$morf_fra}{$lemma_fra} = 'Bahamas__np';
			} elsif ($dix_oci{$morf_oci}{$lemma_oci} eq 'Estats_Units__np') {
				printf $ffra "<e lm=\"%s\"$a><i>%s</i><par n=\"%s\"/></e>\n", $lemma_fra, $stem_fra, 'États-Unis__np';
				$dix_fra{$morf_fra}{$lemma_fra} = 'États-Unis__np';
			} else {
				print STDERR "0b. Falta fra $lemma_fra <$morf_fra> ($lemma_oci: $dix_oci{$morf_oci}{$lemma_oci}: (1), l. $n_linia\n";
				return 0;
			}
			...
			if ($dix_oci{$morf_oci}{$lemma_oci} eq 'ABC__np') {
				printf $ffra "<e lm=\"%s\"$a><i>%s</i><par n=\"%s\"/></e>\n", $lemma_fra, $stem_fra, 'Carrefour__np';
				$dix_fra{$morf_fra}{$lemma_fra} = 'Carrefour__np';
			} elsif ($dix_oci{$morf_oci}{$lemma_oci} eq 'Wikipedia__np') {
				printf $ffra "<e lm=\"%s\"$a><i>%s</i><par n=\"%s\"/></e>\n", $lemma_fra, $stem_fra, 'Wikipedia__np';
				$dix_fra{$morf_fra}{$lemma_fra} = 'Wikipedia__np';
			} elsif ($dix_oci{$morf_oci}{$lemma_oci} eq 'Linux__np') {
				printf $ffra "<e lm=\"%s\"$a><i>%s</i><par n=\"%s\"/></e>\n", $lemma_fra, $stem_fra, 'Linux__np';
				$dix_fra{$morf_fra}{$lemma_fra} = 'Linux__np';
			} elsif ($dix_oci{$morf_oci}{$lemma_oci} eq 'Jeux_olympiques__np') {
				printf $ffra "<e lm=\"%s\"$a><i>%s</i><par n=\"%s\"/></e>\n", $lemma_fra, $stem_fra, 'Jeux_olympiques__np';
				$dix_fra{$morf_fra}{$lemma_fra} = 'Jeux_olympiques__n';
			} elsif ($dix_oci{$morf_oci}{$lemma_oci} eq 'Nacions_Unidas__np') {
				printf $ffra "<e lm=\"%s\"$a><i>%s</i><par n=\"%s\"/></e>\n", $lemma_fra, $stem_fra, 'Nations_unies__np';
				$dix_fra{$morf_fra}{$lemma_fra} = 'Nations_unies__np';
			} else {
				print STDERR "0c. Falta fra $lemma_fra <$morf_fra> ($lemma_oci: $dix_oci{$morf_oci}{$lemma_oci}: (1), l. $n_linia\n";
				return 0;
			}
			...
			if ($dix_oci{$morf_oci}{$lemma_oci} eq 'Antòni__np') {
				printf $ffra "<e lm=\"%s\"$a><i>%s</i><par n=\"%s\"/></e>\n", $lemma_fra, $stem_fra, 'Antoine__np';
				$dix_fra{$morf_fra}{$lemma_fra} = 'Antoine__np';
			} else {
				print STDERR "0d. Falta fra $lemma_fra <$morf_fra> ($lemma_oci: $dix_oci{$morf_oci}{$lemma_oci}: (1), l. $n_linia\n";
				return 0;
			}
		} elsif ($morf_oci eq 'antf') {
			if ($dix_oci{$morf_oci}{$lemma_oci} eq 'Aitana__np') {
				printf $ffra "<e lm=\"%s\"$a><i>%s</i><par n=\"%s\"/></e>\n", $lemma_fra, $stem_fra, 'Marie__np';
				$dix_fra{$morf_fra}{$lemma_fra} = 'Marie__np';
			} else {
				print STDERR "0e. Falta fra $lemma_fra <$morf_fra> ($lemma_oci: $dix_oci{$morf_oci}{$lemma_oci}: (1), l. $n_linia\n";
				return 0;
			}
		} else {
...
die "No estem preparats per GEN_OCI i varietat_oci" if $GEN_OCI && $varietat_oci;
	if ($GEN_OCI && !$dix_oci{$morf_oci}{$lemma_oci}) {
		if ($morf_oci eq 'adj') {
			...
			if ($GEN_FRA && 0) {	# cas expecional de generació
				my $stem_oci = $lemma_oci;
				$stem_oci =~ s| |<b/>|og;
				my $a = " a=\"$autor\"" if $autor;
			...
			if ($dix_fra{$morf_fra}{$lemma_fra} eq 'Carrefour__np') {
				printf $foci "<e lm=\"%s\"$a><i>%s</i><par n=\"%s\"/></e>\n", $lemma_oci, $stem_oci, 'ABC__np';
				$dix_oci{$morf_oci}{$lemma_oci} = 'ABC__np';
			} elsif ($dix_fra{$morf_fra}{$lemma_fra} eq 'Wikipedia__np') {
				printf $foci "<e lm=\"%s\"$a><i>%s</i><par n=\"%s\"/></e>\n", $lemma_oci, $stem_oci, 'Wikipedia__np';
				$dix_oci{$morf_oci}{$lemma_oci} = 'Wikipedia__np';
			} elsif ($dix_fra{$morf_fra}{$lemma_fra} eq 'Linux__np') {
				printf $foci "<e lm=\"%s\"$a><i>%s</i><par n=\"%s\"/></e>\n", $lemma_oci, $stem_oci, 'Linux__np';
				$dix_oci{$morf_oci}{$lemma_oci} = 'Linux__np';
			} elsif ($dix_fra{$morf_fra}{$lemma_fra} eq 'Jeux_olympiques__np') {
			...
			if ($lemma_oci =~ /izar$/o) {
				...
				$dix_oci{$morf_oci}{$lemma_oci} = 1;
			} elsif ($lemma_oci =~ /essar$/o) {
				$stem_oci =~ s/essar$//o;
				printf $foci "<e lm=\"%s\"$a><i>%s</i><par n=\"cap/e[R]ar__vblex\" prm=\"ss\"/></e>\n",  $lemma_oci, $stem_oci;
				$dix_oci{$morf_oci}{$lemma_oci} = 1;
			} elsif ($lemma_oci =~ /ugar$/o) {
				$stem_oci =~ s/ugar$//o;
				printf $foci "<e lm=\"%s\"$a><i>%s</i><par n=\"p/u[R]gar__vblex\" prm=\"\"/></e>\n",  $lemma_oci, $stem_oci;
				$dix_oci{$morf_oci}{$lemma_oci} = 1;
			} elsif ($lemma_oci =~ /ausar$/o) {
				$stem_oci =~ s/ausar$//o;
				printf $foci "<e lm=\"%s\"$a><i>%s</i><par n=\"c/a[NT]ar__vblex\" prm=\"us\"/></e>\n",  $lemma_oci, $stem_oci;
				$dix_oci{$morf_oci}{$lemma_oci} = 1;
			} elsif ($lemma_oci =~ /a([bdflmnprstvxz])ar$/o) {
				my $prm = $1;
				$stem_oci =~ s/a.ar$//o;
				printf $foci "<e lm=\"%s\"$a><i>%s</i><par n=\"c/a[NT]ar__vblex\" prm=\"$prm\"/></e>\n",  $lemma_oci, $stem_oci;
				$dix_oci{$morf_oci}{$lemma_oci} = 1;
			} elsif ($lemma_oci =~ /a([bcdfglmnprstvxz][bdflmnprstvxz])ar$/o) {
				my $prm = $1;
				$stem_oci =~ s/a..ar$//o;
				printf $foci "<e lm=\"%s\"$a><i>%s</i><par n=\"c/a[NT]ar__vblex\" prm=\"$prm\"/></e>\n",  $lemma_oci, $stem_oci;
				$dix_oci{$morf_oci}{$lemma_oci} = 1;
			} elsif ($lemma_oci =~ /a([bcdfglmnprstvxz][bcdfglmnprstvxz][bdflmnprstvxz])ar$/o) {
				my $prm = $1;
				$stem_oci =~ s/a...ar$//o;
				printf $foci "<e lm=\"%s\"$a><i>%s</i><par n=\"c/a[NT]ar__vblex\" prm=\"$prm\"/></e>\n",  $lemma_oci, $stem_oci;
				$dix_oci{$morf_oci}{$lemma_oci} = 1;
				...
				$dix_oci{$morf_oci}{$lemma_oci} = 1;
			} elsif ($lemma_oci =~ /entar$/o) {
				$stem_oci =~ s/entar$//o;
				printf $foci "<e lm=\"%s\"$a><i>%s</i><par n=\"lid/e[R]ar__vblex\" prm=\"nt\"/></e>\n",  $lemma_oci, $stem_oci;
				$dix_oci{$morf_oci}{$lemma_oci} = 1;
			} elsif ($lemma_oci =~ /i([dmnstv])ar$/o) {
				my $prm = $1;
				$stem_oci =~ s/i.ar$//o;
				printf $foci "<e lm=\"%s\"$a><i>%s</i><par n=\"ab/i[T]ar__vblex\" prm=\"$prm\"/></e>\n",  $lemma_oci, $stem_oci;
				$dix_oci{$morf_oci}{$lemma_oci} = 1;
			} elsif ($lemma_oci =~ /i(..)ar$/o) {
				my $prm = $1;
				$stem_oci =~ s/i..ar$//o;
				printf $foci "<e lm=\"%s\"$a><i>%s</i><par n=\"ab/i[T]ar__vblex\" prm=\"$prm\"/></e> VERIFICAR\n",  $lemma_oci, $stem_oci;
				$dix_oci{$morf_oci}{$lemma_oci} = 1;
			} elsif ($lemma_oci =~ /u([bdlmprst])ar$/o) {
				my $prm = $1;
				$stem_oci =~ s/u.ar$//o;
				printf $foci "<e lm=\"%s\"$a><i>%s</i><par n=\"ab/u[S]ar__vblex\" prm=\"$prm\"/></e>\n",  $lemma_oci, $stem_oci;
				$dix_oci{$morf_oci}{$lemma_oci} = 1;
			} elsif ($lemma_oci =~ /u(..)ar$/o) {
				my $prm = $1;
				$stem_oci =~ s/u..ar$//o;
				printf $foci "<e lm=\"%s\"$a><i>%s</i><par n=\"ab/u[S]ar__vblex\" prm=\"$prm\"/></e> VERIFICAR\n",  $lemma_oci, $stem_oci;
				$dix_oci{$morf_oci}{$lemma_oci} = 1;
...
#print STDERR "1. escriure_bidix ($lemma_oci, $stem_oci, $morf_oci, $lemma_fra, $stem_fra, $morf_fra, 'i=\"yes\"', $autor)\n" if $lemma_fra eq 'rifle';
					escriure_bidix ($lemma_oci, $stem_oci, $morf_oci, $varietat_oci, $lemma_fra, $stem_fra, $morf_fra, 'i="yes"', $autor);
					return;
				}
			} else {
...
#print STDERR "2. escriure_bidix ($lemma_oci, $stem_oci, $morf_oci, $lemma_fra, $stem_fra, $morf_fra, 'r=\"LR\"', $autor)\n" if $lemma_fra eq 'rifle';
				escriure_bidix ($lemma_oci, $stem_oci, $morf_oci, $varietat_oci, $lemma_fra, $stem_fra, $morf_fra, $direccio, $autor);
				return;
			}
		}
...
print "3. escriure_bidix ($lemma_oci, $stem_oci, $morf_oci, $varietat_oci, $lemma_fra, $stem_fra, $morf_fra, 'r=\"RL\"', $autor)\n" if $MOT && ($lemma_oci =~ /$MOT/o || $lemma_fra =~ /$MOT/o);
#print STDERR "3. escriure_bidix ($lemma_oci, $stem_oci, $morf_oci, $lemma_fra, $stem_fra, $morf_fra, 'r=\"RL\"', $autor)\n" if $lemma_fra eq 'rifle';
				escriure_bidix ($lemma_oci, $stem_oci, $morf_oci, $varietat_oci, $lemma_fra, $stem_fra, $morf_fra, 'r="RL"', $autor);
				return;
			} else {
...
print "4. escriure_bidix ($lemma_oci, $stem_oci, $morf_oci, $varietat_oci, $lemma_fra, $stem_fra, $morf_fra, '', $autor)\n" if $lemma_fra eq 'rifle';
#print STDERR "4. escriure_bidix ($lemma_oci, $stem_oci, $morf_oci, $varietat_oci, $lemma_fra, $stem_fra, $morf_fra, '', $autor)\n" if $lemma_fra eq 'rifle';
...
#print "99. $. dades[1] = $dades[1]\n" if length $dades[1] == 1;	# una sola lletra
	...
	my $gram_oci = $dades[2];
print "gram_oci = $gram_oci, $linia\n" if $MOT && ($lemma_oci eq $MOT || $lemma_oci eq $MOT);
next if $gram_oci !~ /<$MORF_TRACT>/o && $MORF_TRACT !~ /^ant/o;
next if $gram_oci !~ /<ant>/o && $MORF_TRACT =~ /^ant/o;
...
print "autor = $autor\n" if $MOT && ($lemma_oci eq $MOT || $lemma_fra eq $MOT);
	...
	my $varietat = $dades[5];
	if ($varietat) {
		if ($varietat eq 'gascó') {
			$varietat = 'oci@gascon';
		} elsif ($varietat eq 'aranés') {
			$varietat = 'oci@aran';
		} else {
			print STDERR "Error l. $., varietat = $varietat\n";
			next;
		}
	}

print "11. $linia - stem_oci=$stem_oci, lemma_oci=$lemma_oci, gram_oci = $gram_oci, dades[3]=$dades[3]\n" if $MOT && ($lemma_oci =~ /$MOT/o || $lemma_fra eq $MOT);
...
#print "12a. $linia - stem_oci=$stem_oci, lemma_oci=$lemma_oci, gram_oci = $gram_oci\n" if $MOT && ($lemma_oci =~ /$MOT/o || $lemma_fra eq $MOT);
			$gram_oci = 'n' if $gram_oci =~ /^n>/o;
			$gram_oci = 'top' if $gram_oci =~ /^np><top/o;
			$gram_oci = 'al' if $gram_oci =~ /^np><al/o;
			$gram_oci = 'ant' if $gram_oci =~ /^np><ant/o;
		...
		if ($gram_fra) {
			...
			if ($gram_fra =~ /></o) {
				...
				$gram_fra = 'al' if $gram_fra =~ /^np><al/o;
...
#print "12. $linia - stem_fra=$stem_fra, lemma_fra=$lemma_fra, gram_oci = $gram_oci, gram_fra = $gram_fra\n";
		...
		if ($MORF_TRACT eq 'antm') {
			unless ($dix_oci{antm}{$lemma_oci}) {
				# No és antm
				unless ($dix_oci{antf}{$lemma_oci}) {
					# Hi ha algun error perquè no és tampoc antf
#					print STDERR "4. Falta oci $lemma_oci <$gram_oci> (2), l. $.\n";
				}
				next;
			}
...
#					print STDERR "5. Falta oci $lemma_oci <$gram_oci> (2), l. $.\n";
				}
				next;
			}
		...
		tractar_parella ($lemma_oci, $stem_oci, $gram_oci, $varietat, $lemma_fra, $stem_fra, $gram_fra, $autor, $primer, $.);
		$primer = 0;
	}
